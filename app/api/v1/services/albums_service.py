...
async def get_albums_by_artist_service(artist_id: str) -> list[AlbumByArtist]:
    try:
        collection = await get_collection_db("albums")

        albums = []
        async for document in collection.find({"artist_id": artist_id}):

            album = AlbumByArtist(id=str(document["_id"]), **document)
            albums.append(album)

        return albums

    except Exception as e:
        raise Exception(f"Error retrieving album with the artist: {artist_id}. {str(e)}")
